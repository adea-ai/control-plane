...
export const RetentionJournalOperationSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    kind: z.literal('postgres.deleteEvaluationRun'),
    evalRunId: z.string().min(1).max(256),
  }),
  z.object({
    kind: z.literal('postgres.deleteReleaseAuditRecord'),
    releaseAuditId: z.string().min(1).max(36),
  }),
  z.object({
    kind: z.literal('postgres.deleteExecutionPlan'),
    executionPlanId: z.string().min(1).max(30),
  }),
  z.object({
    kind: z.literal('postgres.deleteInteractionReceipt'),
    commandKey: z.string().min(1).max(64),
  }),
  z.object({
    kind: z.literal('postgres.deleteCancellationReceipt'),
    commandKey: z.string().min(1).max(64),
  }),
  z.object({
    kind: z.literal('postgres.deleteRuntimeCommand'),
    commandId: z.string().min(1).max(30),
  }),
  z.object({
