...
delete result.data.expectedVersion
const metadata = {
  serviceName: 'control-api',
  version: 'test',
  commitSha: 'test',
  environment: 'test',
  instanceId: 'interaction-test',
}
async function withApp(options, run) {
  const calls = []
  const claims = {
    audience: 'control-plane',
    credentialId: 'credential-interaction-test',
...
const inject = (
  application,
  payload = request,
  headers = { authorization: 'Bearer valid-interaction-token' }
) => application.inject({ method: 'POST', url: '/v1/interactions/respond', headers, payload })

test('versioned interaction route requires a scoped authenticated caller and returns signal acceptance', async () =>
  withApp({}, async (application, calls) => {
    expect(createOpenApiDocument(application).paths['/v1/interactions/respond']).toBeDefined()
...
])('wrong credential scope cannot invoke the interaction service %#', async (claims) =>
...
test.each([
  ['INTERACTION_COMMAND_CALLER_MISMATCH', 403],
  ['INTERACTION_DELIVERY_SCOPE_REJECTED', 403],
  ['INTERACTION_COMMAND_PAYLOAD_CONFLICT', 409],
  ['INTERACTION_DELIVERY_EXECUTION_INACTIVE', 409],
...
test('profiles without durable interaction service fail closed', async () =>
