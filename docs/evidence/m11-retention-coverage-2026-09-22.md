...
| Command inbox                      | PostgreSQL/SQLite deletion methods and separate `retireExpiredCommand` rejection-key primitives exist; `deleteEligibleInbox` now deletes only revalidated, retired terminal records and keeps the rejection key (operator-invoked, dry-run default). | Scheduled deletion wiring per profile, execution-events parity, holds, restore-time reapplication, and deployed-profile evidence for the deletion path.                                      |
...
3. Configure missing class policies with the owning product/operator; retain and
   ...
   **Done 2026-09-24:** the per-class durations and hold owners are decided and recorded
   ...
   authoritative predicate plus per-class assessment (command inbox and execution
   events, PostgreSQL and SQLite) with sweep-level reporting landed in the same pass. Eligibility is still
